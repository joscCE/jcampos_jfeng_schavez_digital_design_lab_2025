module Check_win(
    ...
    output logic [1:0]  win_type,     // 00=horizontal, 01=vertical, 10=diagonal \, 11=diagonal /
    output logic [2:0]  win_base_col, // Columna base de la secuencia ganadora
    output logic [2:0]  win_base_row, // Fila base de la secuencia ganadora
    output logic [1:0]  win_length    // Longitud de la secuencia (útil para direcciones)
);

    logic [2:0] col;                  // Columna del movimiento
    logic [2:0] fila;                 // Fila del movimiento (declarado como fila, no row)
    ...
    assign fila = play_made[2:0];     // Fila del movimiento (asignado a fila)
    ...
    always_comb begin
        ...
        win_type = 2'b00;
        win_base_col = col;
        win_base_row = fila;  // Cambiado de row a fila
        win_length = 4; // Por defecto 4 en raya

        // Determinar dirección de la victoria
        if (win_horiz) begin
            win_type = 2'b00;
            // Encontrar el inicio de la secuencia horizontal
            for (int i = 0; i < 4; i++) begin
                if (col-i >= 0 && cell_matches(col-i, fila)) begin  // Cambiado row por fila
                    win_base_col = col-i;
                end
            end
        end
        else if (win_vert) begin
            win_type = 2'b01;
            win_base_row = fila-3; // Cambiado de row a fila
        end
        else if (win_diag1) begin
            win_type = 2'b10;
            // Encontrar el inicio de la diagonal \
            for (int i = 0; i < 4; i++) begin
                if (col-i >= 0 && fila-i >= 0 && cell_matches(col-i, fila-i)) begin  // Cambiado row por fila
                    win_base_col = col-i;
                    win_base_row = fila-i;  // Cambiado de row a fila
                end
            end
        end
        else if (win_diag2) begin
            win_type = 2'b11;
            // Encontrar el inicio de la diagonal /
            for (int i = 0; i < 4; i++) begin
                if (col+i <= 6 && fila-i >= 0 && cell_matches(col+i, fila-i)) begin  // Cambiado row por fila
                    win_base_col = col+i;
                    win_base_row = fila-i;  // Cambiado de row a fila
                end
            end
